		...
		for (i; i < args.length; i++) {
			...
			if ($filter_selector.length === 1) {
				...
				if (args[i].filter_type === "select") {
					...
					if (filter_reset_button_text !== false) {
						$(filter_selector_string).find(".yadcf-filter").after("<button style=\"margin-bottom:10px\" value=\"" + filter_reset_button_text + "\" type=\"button\" " +
							"onclick=\"$(\'.sel"+ filter_container_id.charAt(1) +"\').selectpicker('val', 'Select value'); $(\'.sel"+ filter_container_id.charAt(1) +"\').selectpicker('deselectAll'); yadcf.stopPropagation(event);yadcf.doFilter('clear', '" + table_selector_jq_friendly + "', " + column_number + "); return false;\" class=\"btn btn-default\"><span class=\"glyphicon glyphicon-remove\"></span></button>");
					}
				...
				//	args[i].select_type_options = $.extend({}, {width: $(filter_selector_string).closest("th").width() + "px"}, args[i].select_type_options);
					...
					if (filter_reset_button_text !== false) {
						$(filter_selector_string).find(".yadcf-filter").after("<button style=\"margin-bottom:10px\" value=\"" + filter_reset_button_text + "\" type=\"button\" " +
							"onclick=\"$(\'.sel"+ filter_container_id.charAt(1) +"\').selectpicker('val', 'Select value'); $(\'.sel"+ filter_container_id.charAt(1) +"\').selectpicker('deselectAll'); yadcf.stopPropagation(event);yadcf.doFilterAutocomplete('clear', '" + table_selector_jq_friendly + "', " + column_number + "); return false;\" class=\"btn btn-default\"><span class=\"glyphicon glyphicon-remove\"></span></button>");
					}
					...
					if (filter_reset_button_text !== false) {
						$(filter_selector_string).find(".yadcf-filter").after("<button style=\"margin-bottom:10px\" value=\"" + filter_reset_button_text + "\" type=\"button\" " + " id=\"yadcf-filter-" + table_selector_jq_friendly + "-" + column_number + "-reset\"" +
							"onclick=\"$(\'.sel"+ filter_container_id.charAt(1) +"\').selectpicker('val', 'Select value'); $(\'.sel"+ filter_container_id.charAt(1) +"\').selectpicker('deselectAll'); yadcf.stopPropagation(event);yadcf.textKeyUP('" + table_selector_jq_friendly + "', event, 'clear'); return false;\" class=\"btn btn-default\"><span class=\"glyphicon glyphicon-remove\"></span></button>");
					}
